...
/**
 * End-to-End Test: Snowflake Connector
 *
 * Tests the complete flow from schema → data generation → WireMock → MCP tools
 *
 * Run with: npx ts-node scripts/test-snowflake-e2e.ts
 */
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════════╗');
  console.log('║           SNOWFLAKE CONNECTOR END-TO-END TEST                      ║');
  console.log('╚════════════════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  await runTest('Snowflake schema exists', async () => {
    const schemaPath = path.join(process.cwd(), 'schemas', 'snowflake.json');
  ...
  await runTest('Factory pipeline executes successfully', async () => {
    const input: PipelineInput = {
      ...
      schemaPath: path.join(process.cwd(), 'schemas', 'snowflake.json'),
      industry: 'technology',
      volume: 20, // Small volume for testing
  ...
  await runTest('Schema converted to OpenAPI', async () => {
    const openApiPath = path.join(outputDir, 'schemas', 'snowflake_openapi.yaml');
    const stat = await fs.stat(openApiPath);
    return { path: openApiPath, size: stat.size };
  });

  await runTest('Data files generated', async () => {
    const dataDir = path.join(outputDir, 'data', 'snowflake');
    const files = await fs.readdir(dataDir).catch(() => []);
    return { dataDir, fileCount: files.length, files };
  });

  await runTest('Credentials generated', async () => {
    const credsPath = path.join(outputDir, 'credentials', 'snowflake_creds.json');
  ...
  // ========================================================================
  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  STAGE 3: MCP Tool Verification');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

  await runTest('list_databases returns data', async () => {
  ...
  await runTest('execute_sql works', async () => {
    ...
    return {
      status: response.status,
      hasStatementHandle: !!response.data.statementHandle,
      code: response.data.code,
      message: response.data.message,
    };
  });

  await runTest('list_tables works', async () => {
    const response = await axios.get(`${WIREMOCK_URL}/api/v2/tables`, {
      params: { database: 'SAMPLE_DATA', schema: 'PUBLIC' },
    });
    return {
      status: response.status,
  ...
  await runTest('Stateful scenario: Initial state', async () => {
    ...
    return {
      scenarios: response.data.scenarios?.map((s: { name: string; state: string }) => ({
        name: s.name,
        state: s.state,
      })),
    };
